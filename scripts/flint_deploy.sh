...
# Rust 传入的环境变量:
#   GITLAB_PROJECT   e.g. "dev-team/flint"
...
DOCKERFILE="${SRC_DIR}/flint/Dockerfile"
...
# 将基础镜像改为 ubuntu:24.04（宿主机编译的二进制需要 GLIBC 2.39）
sed -i 's|FROM debian:bookworm-slim|FROM ubuntu:24.04|g' "${DOCKERFILE}"
